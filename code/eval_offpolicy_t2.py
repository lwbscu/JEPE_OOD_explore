#!/usr/bin/env python3
"""Evaluate the frozen T2 planner with the off-policy-finetuned checkpoint.

This entry point intentionally imports the proven T2 solver, injection proxy,
retrieval policy, RNG derivation, selector, and artifact writers.  The only
experimental factor is the world-model checkpoint.  A formal 50-environment
run additionally requires the all-color aggregate offline PASS gate produced
by ``tools/evaluate_cube_offpolicy.py``.
"""
...
import evaluate_cube_offpolicy as offline  # noqa: E402
...
def _default_output(condition: str, num_eval: int) -> Path:
    return (
        ...
        if num_eval == 50
        else OUTPUT_ROOT / "smoke" / condition
    )
...
def _checkpoint_provenance(checkpoint: Path) -> dict[str, Any]:
    ...
    if not config.is_file():
        raise FileNotFoundError(f"off-policy checkpoint config missing: {config}")
    return {
        "kind": "offpolicy_v1_predictor_finetune",
        "run_id": resolved.parent.name,
        "weights": offline._identity(resolved),
        "config": offline._identity(config),
    }
...
def run(args: argparse.Namespace) -> int:
    ...
    output = _prepare_output(
        args.output or _default_output(args.condition, args.num_eval), args.overwrite
    )

    # These are the exact existing T2 implementation objects.  No local solver,
    # injection, retrieval, selector, or RNG behavior is introduced here.
    ...
    payload = {
        "format_version": "cube_offpolicy_t2_evaluation_v1",
        "protocol": {
            "id": PROTOCOL,
