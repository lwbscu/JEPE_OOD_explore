#!/usr/bin/env python3
"""Offline all-AND gate for the Cube off-policy V3 predictor.

V3 is evaluated on two frozen, disjoint protocols:
...
The MaskedAug encoder/projector and XYZ probe are reusable only when the V3
...
CHECKPOINT_ROOT = AILAB_ROOT / "checkpoints/lewm-cube-offpolicy_v3"
DEFAULT_CHECKPOINT = CHECKPOINT_ROOT / "offpolicy_v3_pred_seed3072/weights_final.pt"
OUTPUT_ROOT = AILAB_ROOT / "outputs/eval/cube/offpolicy_v3"
OFFLINE_ROOT = OUTPUT_ROOT / "offline"
TRAIN_OUTPUT_ROOT = AILAB_ROOT / "outputs/train/offpolicy_v3"
...
# before V3 was designed; the displayed percentages are 31.222/38.500/38.514.
RATE_THRESHOLDS = {
...
FORMAL_ARM = "v3_96E_32V2_expert_rollout_disabled"
FORMAL_GLOBAL_STEP = 5_000
STOPLINE_BASELINE = 0.0032953384798020124
...
V2_DATA_ROOT = AILAB_ROOT / "datasets/offpolicy_cube_v2"
V2_MANIFEST = V2_DATA_ROOT / "manifest.json"
V2_VALIDATION = V2_DATA_ROOT / "validation.json"
V2_MANIFEST_SHA256 = "6fe81e3a98fe2327f206ddea3dcad32d22667d5348654f6c46803622a3cb7ce3"
V2_VALIDATION_SHA256 = "f4e948e273529db805e867518e8fba6f870d628fa90163474c6439ab7ab54077"
V2_EPISODE_SPLIT = (
    AILAB_ROOT
    / "outputs/train/offpolicy_v2/offpolicy_v2_pred_seed3072/episode_split.npz"
)
V2_RUN_PLAN_REFERENCE = (
    AILAB_ROOT / "outputs/train/offpolicy_v2/offpolicy_v2_pred_seed3072/run_plan.json"
)
TRAINABLE_SCOPE = {
    "frozen_integrity_modules": ["encoder.", "projector."],
    "frozen_parameter_tensors": 204,
    "frozen_parameters": 6_294_144,
    "trainable_parameter_tensors": 93,
    "trainable_parameters": 11_740_484,
    "trainable_prefixes": ["predictor.", "action_encoder.", "pred_proj."],
}
FROZEN_SUBSTATE_SHA256 = {
    "encoder": "cce9c72c62c70bacd57cffb2e5a49d9114e81b008b12ca4dacd32f48a10617a2",
    "projector": "e6d7283d7c44effdb9aaf409a21306d1450f588ea6590015ad30e4f3bb9455d6",
}
...
LOSS_CONTRACT_SHA256 = _canonical_json_sha256(LOSS_CONTRACT)
...
def _checkpoint(path: Path) -> Path:
    ...
    if root not in resolved.parents or resolved.name != "weights_final.pt":
        raise ValueError(f"V3 checkpoint must be weights_final.pt below {root}: {resolved}")
    if not resolved.is_file():
        raise FileNotFoundError(resolved)
    if resolved != DEFAULT_CHECKPOINT.resolve() or resolved.parent.name != FORMAL_RUN_ID:
        raise ValueError(
            "V3 has exactly one formal arm/checkpoint: "
            f"expected={DEFAULT_CHECKPOINT.resolve()}, actual={resolved}"
        )
    return resolved
...
def _stopline_provenance_matches(value: Mapping[str, Any]) -> bool:
    return set(value) == {
        "num_batches",
        "examples",
        "batch_size",
        ...
        "drop_last": True,
        "precision": "bf16",
        "expert_clip_indices_sha256": V2_FORMAL_PROVENANCE_SHA256,
        "episode_split_sha256": V2_FORMAL_SPLIT_SHA256,
    }


def _validate_stopline_history(history: Mapping[str, Any]) -> dict[str, Any]:
    expected_top = {
        "format_version",
        "run_id",
        "arm",
    ...
    if (
        set(history) != expected_top
        or history.get("format_version") != "cube_offpolicy_v3_stopline_history_v1"
        or history.get("run_id") != FORMAL_RUN_ID
        or history.get("arm") != FORMAL_ARM
        or float(history.get("threshold", np.nan)) != EXPERT_STOPLINE
        or history.get("comparison")
        != "relative strictly greater than threshold triggers"
        ...
        raise RuntimeError("V3 stopline history top-level formal PASS contract failed")
    baseline = history.get("baseline", {})
    if (
        set(baseline) != {"expected", "current", "step", "relative", "status", "provenance"}
        or float(baseline.get("expected", np.nan)) != STOPLINE_BASELINE
        or float(baseline.get("current", np.nan)) != STOPLINE_BASELINE
        or int(baseline.get("step", -1)) != 0
        or float(baseline.get("relative", np.nan)) != 0.0
        or baseline.get("status") != "PASS"
        or not _stopline_provenance_matches(baseline.get("provenance", {}))
    ):
        raise RuntimeError("V3 exact step-zero expert baseline/provenance contract failed")
    records = history.get("records", [])
    expected_steps = list(range(STOPLINE_INTERVAL, FORMAL_GLOBAL_STEP + 1, STOPLINE_INTERVAL))
    if not isinstance(records, list) or len(records) != len(expected_steps):
        raise RuntimeError(
            f"V3 stopline records incomplete: expected_steps={expected_steps}, actual_count={len(records)}"
        )
    normalized = []
    for expected_step, record in zip(expected_steps, records, strict=True):
        if set(record) != {"step", "current", "relative", "status", "provenance"}:
            raise RuntimeError(f"V3 stopline record schema changed at step {expected_step}")
        current = float(record.get("current", np.nan))
        relative = float(record.get("relative", np.nan))
        recomputed = current / STOPLINE_BASELINE - 1.0
        if (
            int(record.get("step", -1)) != expected_step
            or not np.isfinite([current, relative]).all()
            or current < 0
            or not np.isclose(relative, recomputed, atol=1e-12, rtol=1e-12)
            or relative > EXPERT_STOPLINE
            or record.get("status") != "PASS"
            or record.get("provenance") != baseline["provenance"]
        ):
            raise RuntimeError(
                f"V3 stopline record is non-paired/non-PASS: step={expected_step}, record={record}"
            )
        normalized.append(
            {"step": expected_step, "current": current, "relative_recomputed": recomputed}
        )
    return {
        "baseline": STOPLINE_BASELINE,
        "threshold": EXPERT_STOPLINE,
...
def _validate_run_plan(run_plan: Mapping[str, Any]) -> dict[str, Any]:
    """Independently reproduce every user-frozen V3 training input/config."""
    if not all(path.is_file() for path in (NORMALIZERS, V2_MANIFEST, V2_VALIDATION, V2_EPISODE_SPLIT)):
        raise FileNotFoundError("one or more frozen V3 input-contract files are missing")
    actual_hashes = {
        "masked_checkpoint": v1._sha256(BASE_CHECKPOINT),
        "normalizers": v1._sha256(NORMALIZERS),
        "v2_manifest": v1._sha256(V2_MANIFEST),
        "v2_validation": v1._sha256(V2_VALIDATION),
        "v2_episode_split": v1._sha256(V2_EPISODE_SPLIT),
        "formal_manifest": v1._sha256(MANIFEST),
        "measurement1_segments": v1._sha256(EXPERT_SEGMENTS),
    }
    expected_hashes = {
        "masked_checkpoint": MASKED_CHECKPOINT_SHA256,
        "normalizers": NORMALIZERS_SHA256,
        "v2_manifest": V2_MANIFEST_SHA256,
        "v2_validation": V2_VALIDATION_SHA256,
        "v2_episode_split": V2_FORMAL_SPLIT_SHA256,
        "formal_manifest": actual_hashes["formal_manifest"],
        "measurement1_segments": actual_hashes["measurement1_segments"],
    }
    if actual_hashes != expected_hashes:
        raise RuntimeError(
            f"frozen V3 input hashes changed: expected={expected_hashes}, actual={actual_hashes}"
        )
    inputs = run_plan.get("inputs", {})
    split_identity = inputs.get("formal_v2_episode_split", {})
    collector = inputs.get("collector_validation", {})
    collector_manifest = collector.get("manifest", {})
    counters = collector.get("semantic_counters", {})
    revalidation = collector.get("semantic_revalidation", {})
    if (
        Path(inputs.get("expert_dataset", "")).resolve() != DATASET.resolve()
        or Path(inputs.get("formal_manifest", "")).resolve() != MANIFEST.resolve()
        or inputs.get("formal_manifest_sha256") != actual_hashes["formal_manifest"]
        or Path(inputs.get("measurement1_segments", {}).get("path", "")).resolve()
        != EXPERT_SEGMENTS.resolve()
        or inputs.get("measurement1_segments", {}).get("sha256")
        != actual_hashes["measurement1_segments"]
        or int(inputs.get("measurement1_segments", {}).get("num_segments", -1)) != EXPERT_COUNT
        or Path(inputs.get("v2_root", "")).resolve() != V2_DATA_ROOT.resolve()
        or Path(inputs.get("v2_manifest", "")).resolve() != V2_MANIFEST.resolve()
        or inputs.get("v2_manifest_sha256") != V2_MANIFEST_SHA256
        or Path(inputs.get("warm_start", "")).resolve() != BASE_CHECKPOINT.resolve()
        or inputs.get("warm_start_sha256") != MASKED_CHECKPOINT_SHA256
        or Path(inputs.get("normalizers", "")).resolve() != NORMALIZERS.resolve()
        or inputs.get("normalizers_sha256") != NORMALIZERS_SHA256
        or Path(split_identity.get("path", "")).resolve() != V2_EPISODE_SPLIT.resolve()
        or split_identity.get("sha256") != V2_FORMAL_SPLIT_SHA256
        or int(split_identity.get("size_bytes", -1)) != V2_EPISODE_SPLIT.stat().st_size
        or Path(collector.get("path", "")).resolve() != V2_VALIDATION.resolve()
        or collector.get("sha256") != V2_VALIDATION_SHA256
        or Path(collector_manifest.get("path", "")).resolve() != V2_MANIFEST.resolve()
        or collector_manifest.get("sha256") != V2_MANIFEST_SHA256
        or int(collector.get("decoded_jpeg_shards", -1)) != 150
        or len(collector.get("bound_shards", [])) != 150
        or counters.get("valid") is not True
        or counters.get("scope") != "formal"
        or int(counters.get("num_rollouts", -1)) != 31_500
        or int(counters.get("num_source_episodes", -1)) != 150
        or int(counters.get("formal_episode_overlap", -1)) != 0
        or int(counters.get("measurement1_holdout_overlap", -1)) != 0
        or int(counters.get("memory_global_exclusion_overlap", -1)) != 0
        or counters.get("no_extra_action_clip") is not True
        or revalidation.get("action_model_exact_planner_pool") is not True
        or int(revalidation.get("cem_iterations", -1)) != 10
        or int(revalidation.get("complete_pool_candidates_per_iteration", -1)) != 300
        or int(revalidation.get("replayed_candidates_per_iteration", -1)) != 21
        or revalidation.get("float32_inverse_exact") is not True
        or int(revalidation.get("retrieval_exclusion_overlap", -1)) != 0
    ):
        raise RuntimeError("run_plan V2 dataset/input/validation contract is not exact")
    bound = inputs.get("collector_bound_inputs", {})
    for name, path, sha in (
        ("formal_manifest", MANIFEST, actual_hashes["formal_manifest"]),
        ("masked_checkpoint", BASE_CHECKPOINT, MASKED_CHECKPOINT_SHA256),
        ("masked_config", BASE_CHECKPOINT.parent / "config.json", v1._sha256(BASE_CHECKPOINT.parent / "config.json")),
        ("measurement1_segments", EXPERT_SEGMENTS, actual_hashes["measurement1_segments"]),
    ):
        identity = bound.get(name, {})
        if Path(identity.get("path", "")).resolve() != path.resolve() or identity.get("sha256") != sha:
            raise RuntimeError(f"run_plan collector bound input changed: {name}")
    expected_optimization = {
        "optimizer": "AdamW",
        "learning_rate": 1e-5,
        "weight_decay": 1e-3,
        "steps": FORMAL_GLOBAL_STEP,
    ...
    expected_augmentation = {
        "masked_hue_probability": 0.8,
        "max_hue_delta_turns": 0.5,
    ...
    freeze = run_plan.get("freeze", {})
    freeze_core = {key: freeze.get(key) for key in TRAINABLE_SCOPE}
    if (
        run_plan.get("batch") != expected_batch
        or run_plan.get("optimization") != expected_optimization
        or run_plan.get("expert_augmentation") != expected_augmentation
        or run_plan.get("runtime") != expected_runtime
        or freeze_core != TRAINABLE_SCOPE
        or freeze.get("frozen_sha256_before") != FROZEN_SUBSTATE_SHA256
    ):
        raise RuntimeError("run_plan optimizer/runtime/augmentation/trainable-scope contract changed")
    return {
        "input_sha256": actual_hashes,
        "optimization": expected_optimization,
        "batch": expected_batch,
        "expert_augmentation": expected_augmentation,
        "runtime": expected_runtime,
        "trainable_scope": {**TRAINABLE_SCOPE, "frozen_sha256_before": FROZEN_SUBSTATE_SHA256},
        "collector_validation_sha256": V2_VALIDATION_SHA256,
    }


def _training_contract(checkpoint: Path) -> dict[str, Any]:
    """Validate the unique V3 arm, live stopline, loss, and final posthoc."""
    ...
    history_path = run_root / "stopline_history.json"
    completed_path = run_root / "completed.json"
    posthoc_path = run_root / "posthoc_validation.json"
    for path in (run_plan_path, history_path, completed_path, posthoc_path):
        if not path.is_file() or path.parent != run_root:
            raise FileNotFoundError(f"same-run V3 training artifact missing: {path}")
    ...
    if (
        run_plan.get("format_version") != "cube_offpolicy_train_v3"
        or run_plan.get("run_id") != FORMAL_RUN_ID
        or run_plan.get("arm") != FORMAL_ARM
        or run_plan.get("single_arm") is not True
        or run_plan.get("retry_allowed") is not False
        or run_plan.get("resume_allowed") is not False
        or run_plan.get("loss_contract") != LOSS_CONTRACT
        or run_plan.get("loss_contract_sha256") != LOSS_CONTRACT_SHA256
        or _canonical_json_sha256(run_plan.get("loss_contract")) != LOSS_CONTRACT_SHA256
    ):
        raise RuntimeError("run_plan.json is not the unique formal V3 loss arm")
    run_plan_contract = _validate_run_plan(run_plan)
    history_contract = _validate_stopline_history(history)
    history_identity = v1._identity(history_path)
    completed_history = completed.get("stopline_history", {})
    if (
        completed.get("format_version") != "cube_offpolicy_v3_completed_v1"
        or completed.get("status") != "PASS"
        or completed.get("offline_gate_authorized") is not True
        or completed.get("run_id") != run_id
        or completed.get("arm") != FORMAL_ARM
        or int(completed.get("global_step", -1)) != FORMAL_GLOBAL_STEP
        ...
        or completed.get("stopline_event") is not None
        or completed.get("source_mix") != {"expert": 96, "v2": 32}
        or completed.get("cross_split_episode_overlaps")
        != {
            "expert_train_vs_expert_validation": 0,
            "expert_train_vs_v2_validation": 0,
            "expert_validation_vs_v2_train": 0,
            "v2_train_vs_v2_validation": 0,
        }
        or completed.get("frozen_integrity", {}).get("exact_match") is not True
        or completed.get("frozen_integrity", {}).get("before") != FROZEN_SUBSTATE_SHA256
        or completed.get("frozen_integrity", {}).get("after") != FROZEN_SUBSTATE_SHA256
        or completed.get("loss_contract") != LOSS_CONTRACT
        or completed.get("loss_contract_sha256") != LOSS_CONTRACT_SHA256
        or _canonical_json_sha256(completed.get("loss_contract")) != LOSS_CONTRACT_SHA256
        or Path(completed_history.get("path", "")).resolve() != history_path
        or int(completed_history.get("size_bytes", -1)) != history_identity["size"]
        or completed_history.get("sha256") != history_identity["sha256"]
    ):
        raise RuntimeError("completed.json does not authorize the exact complete formal V3 arm")
    if (
        (run_root / "stopline_event.json").exists()
        or list(checkpoint.parent.glob("weights_stopped_step*.pt"))
        or list((checkpoint.parent / "lightning").glob("stopline_step*.ckpt"))
    ):
        raise RuntimeError("V3 stopline-trigger artifacts exist despite claimed formal PASS")
    if (
        posthoc.get("format_version") != "cube_offpolicy_v3_paired_posthoc_v1"
        ...
        raise RuntimeError("posthoc_validation.json protocol is not paired clean-expert V3")
    ...
    if (
        ...
        raise RuntimeError("posthoc base/final checkpoints do not match Masked/V3 identities")
    ...
    if (
        base_expert.get("provenance_sha256") != V2_FORMAL_PROVENANCE_SHA256
        or int(base_expert.get("num_batches", -1)) != STOPLINE_BATCHES
        or int(base_expert.get("examples", -1)) != STOPLINE_EXAMPLES
        or float(expert_teacher_pred["base"]) != STOPLINE_BASELINE
        or not np.isclose(
            float(expert_teacher_pred["final"]),
            float(history_contract["final"]["current"]),
            atol=0.0,
            rtol=0.0,
        )
        or not np.isclose(
            float(expert_teacher_pred["relative_increase_recomputed"]),
            float(history_contract["final"]["relative_recomputed"]),
            atol=1e-12,
            rtol=1e-12,
        )
    ):
        raise RuntimeError("final posthoc does not reproduce the complete live stopline history")
    return {
        "format_version": "cube_offpolicy_v3_training_provenance_v1",
        "run_id": run_id,
        "arm": FORMAL_ARM,
        "checkpoint": checkpoint_identity,
        "run_plan": v1._identity(run_plan_path),
        "run_plan_contract": run_plan_contract,
        "stopline_history": history_identity,
        "completed": v1._identity(completed_path),
        "posthoc_validation": v1._identity(posthoc_path),
        "loss_contract": {
            "value": LOSS_CONTRACT,
            "sha256": LOSS_CONTRACT_SHA256,
            "expert_rollout_disabled": True,
        },
        "live_stopline": history_contract,
        "expert_teacher_pred": expert_teacher_pred,
    }


def _freeze_probe_contract(checkpoint: Path, output: Path) -> dict[str, Any]:
    """Bind V3 to the unchanged Masked encoder/projector and Masked XYZ probe."""
    import torch
    ...
    if base_keys != new_keys:
        raise RuntimeError("V3 changed the encoder/projector key set; Masked probe reuse is invalid")
    differences = [
        key
    ...
    if differences:
        raise RuntimeError(
            "V3 changed encoder/projector tensors; fresh probe required; "
    ...
    payload = {
        "format_version": "cube_offpolicy_v3_probe_provenance_v1",
        "created_before_outcome_join": True,
        "reuse_reason": (
            "V3 freezes encoder and projector elementwise; one Masked XYZ probe keeps "
...
def _freeze_inputs(checkpoint: Path, output: Path) -> dict[str, Any]:
    ...
    if np.intersect1d(current_segment_episodes, current_formal_episodes).size:
        raise RuntimeError("formal50 episode leaked into fixed V3 expert segments")
    ...
    payload = {
        "format_version": "cube_offpolicy_v3_offline_inputs_frozen_v1",
        "created_before_outcome_join": True,
...
def _measure_expert(
    ...
    summary = {
        "format_version": "cube_offpolicy_v3_expert_measurement_v1",
        "protocol": {
...
def _build_gate(
    ...
    live_gate = training["live_stopline"]
    loss_gate = training["loss_contract"]
    training_pass = bool(
        ...
        <= EXPERT_STOPLINE
        and live_gate.get("triggered") is False
        and live_gate.get("final", {}).get("step") == FORMAL_GLOBAL_STEP
        and float(live_gate.get("final", {}).get("relative_recomputed", np.inf))
        <= EXPERT_STOPLINE
        and loss_gate.get("sha256") == LOSS_CONTRACT_SHA256
        and loss_gate.get("expert_rollout_disabled") is True
    )
    colors: dict[str, Any] = {}
    for condition in CONDITIONS:
    ...
    return {
        "format_version": "cube_offpolicy_v3_aggregate_gate_v1",
        "status": status,
        "authorization": (
        ...
        "colors": colors,
        "training_expert_stopline": {
            **training_gate,
            "live_stopline": live_gate,
            "loss_contract_sha256": loss_gate["sha256"],
            "expert_rollout_disabled": loss_gate["expert_rollout_disabled"],
            "pass": training_pass,
        },
...
def _gate_exit_code(status: str) -> int:
    if status not in {"PASS", "FAIL"}:
        raise ValueError(f"unknown V3 gate status: {status}")
...
def command_run(args: argparse.Namespace) -> int:
    ...
    if v1._identity(checkpoint) != frozen_inputs["new_checkpoint"]:
        raise RuntimeError("V3 checkpoint changed during offline evaluation")
    if v1._identity(checkpoint.parent / "config.json") != frozen_inputs[
        "checkpoint_config_contract"
    ]["new_config"]:
        raise RuntimeError("V3 checkpoint config changed during offline evaluation")
    ...
    summary = {
        "format_version": "cube_offpolicy_v3_offline_summary_v1",
        "protocol": {
...
def _read_candidate_core(scores_path: Path, summary_path: Path) -> dict[str, Any]:
    ...
    if (
        summary.get("format_version") != "cube_offpolicy_v3_offline_summary_v1"
        ...
        raise ValueError("V3 offline summary protocol malformed")
...
def _read_expert_core(csv_path: Path, summary_path: Path) -> dict[str, Any]:
    ...
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    if (
        summary.get("format_version") != "cube_offpolicy_v3_expert_measurement_v1"
...
def _validate_gate(
    ...
    if (
        gate.get("format_version") != "cube_offpolicy_v3_aggregate_gate_v1"
        or gate.get("status") != "PASS"
        or gate.get("all_requirements_AND") is not True
        or gate.get("all_colors_required") is not True
    ):
        raise ValueError(f"aggregate V3 gate is not passing: {gate.get('status')}")
    checkpoint = _checkpoint(checkpoint)
    ...
    if (
        ...
        raise ValueError("gate checkpoint/config differs from requested V3 model")
    required_artifacts = {
        "frozen_inputs.json",
        "probe_provenance.json",
        "training_provenance.json",
    ...
    if (
        wrapper.get("format_version") != "cube_offpolicy_v3_probe_provenance_v1"
        ...
        raise ValueError("V3 probe provenance invalid")
    frozen = json.loads((resolved.parent / "frozen_inputs.json").read_text())
    if (
        frozen.get("format_version") != "cube_offpolicy_v3_offline_inputs_frozen_v1"
        ...
        raise ValueError("V3 frozen inputs invalid")
    ...
    expected_training_gate = {
        **training_current["expert_teacher_pred"],
        "live_stopline": training_current["live_stopline"],
        "loss_contract_sha256": training_current["loss_contract"]["sha256"],
        "expert_rollout_disabled": True,
        "pass": True,
    }
...
def command_self_test(_: argparse.Namespace) -> int:
    ...
    base_expert = {
        "provenance_sha256": V2_FORMAL_PROVENANCE_SHA256,
        "num_batches": STOPLINE_BATCHES,
        "examples": STOPLINE_EXAMPLES,
        "mean": {"teacher_pred_loss": STOPLINE_BASELINE},
    }
    final_loss = STOPLINE_BASELINE * 1.09
    final_expert = {
        "provenance_sha256": V2_FORMAL_PROVENANCE_SHA256,
        "num_batches": STOPLINE_BATCHES,
        "examples": STOPLINE_EXAMPLES,
        "mean": {"teacher_pred_loss": final_loss},
    }
    stopline = {
        "threshold_relative_increase": 0.10,
        "base_teacher_pred_loss": STOPLINE_BASELINE,
        "final_teacher_pred_loss": final_loss,
    ...
    for mutation in ("threshold", "authorization", "relative", "increase"):
        ...
        if mutation == "threshold":
            ...
            bad_final["mean"]["teacher_pred_loss"] = STOPLINE_BASELINE * 1.11
        try:
    ...
    history = {
        ...
        "arm": FORMAL_ARM,
        "baseline": {
            "expected": STOPLINE_BASELINE,
            "current": STOPLINE_BASELINE,
            "step": 0,
            "relative": 0.0,
            "status": "PASS",
        ...
        "records": [
            {
                "step": step,
                "current": STOPLINE_BASELINE * (1.0 + 0.05),
                "relative": 0.05,
                "status": "PASS",
                "provenance": provenance,
            }
            for step in range(STOPLINE_INTERVAL, FORMAL_GLOBAL_STEP + 1, STOPLINE_INTERVAL)
        ],
    }
    history_result = _validate_stopline_history(history)
    history_negative_checks = 0
    for mutation in ("missing_record", "triggered", "baseline", "provenance"):
        bad_history = json.loads(json.dumps(history))
        if mutation == "missing_record":
            bad_history["records"].pop()
        elif mutation == "triggered":
            bad_history["triggered"] = True
        elif mutation == "baseline":
            bad_history["baseline"]["current"] += 1e-12
        else:
            bad_history["records"][0]["provenance"]["expert_clip_indices_sha256"] = "bad"
        try:
            _validate_stopline_history(bad_history)
        except RuntimeError:
            history_negative_checks += 1
    if history_negative_checks != 4:
        raise AssertionError("V3 history negative tests did not fail closed")
    if _canonical_json_sha256(LOSS_CONTRACT) != LOSS_CONTRACT_SHA256:
        raise AssertionError("V3 loss contract canonical hash mismatch")
    plan_fixture = json.loads(V2_RUN_PLAN_REFERENCE.read_text(encoding="utf-8"))
    plan_fixture["inputs"]["formal_v2_episode_split"] = {
        "path": str(V2_EPISODE_SPLIT.resolve()),
        "size_bytes": V2_EPISODE_SPLIT.stat().st_size,
        "sha256": V2_FORMAL_SPLIT_SHA256,
    }
    plan_result = _validate_run_plan(plan_fixture)
    plan_negative_checks = 0
    for mutation in ("lr", "normalizer", "split", "scope", "hue"):
        bad_plan = json.loads(json.dumps(plan_fixture))
        if mutation == "lr":
            bad_plan["optimization"]["learning_rate"] = 5e-6
        elif mutation == "normalizer":
            bad_plan["inputs"]["normalizers_sha256"] = "bad"
        elif mutation == "split":
            bad_plan["inputs"]["formal_v2_episode_split"]["sha256"] = "bad"
        elif mutation == "scope":
            bad_plan["freeze"]["trainable_parameters"] += 1
        else:
            bad_plan["expert_augmentation"]["masked_hue_probability"] = 0.7
        try:
            _validate_run_plan(bad_plan)
        except RuntimeError:
            plan_negative_checks += 1
    if plan_negative_checks != 5:
        raise AssertionError("V3 frozen run-plan negatives did not fail closed")
    if _gate_exit_code("PASS") != 0 or _gate_exit_code("FAIL") != 3:
        raise AssertionError("V3 fail-stop process exit contract failed")
    try:
        _gate_exit_code("UNKNOWN")
    except ValueError:
        pass
    else:
        raise AssertionError("V3 unknown gate status did not fail closed")
    if not all(
        ...
        json.dumps(
            {
                ...
                "live_history_positive": history_result,
                "live_history_negative_checks": history_negative_checks,
                "loss_contract_sha256": LOSS_CONTRACT_SHA256,
                "run_plan_positive": plan_result,
                "run_plan_negative_checks": plan_negative_checks,
                "gate_exit_codes": {"PASS": 0, "FAIL": 3, "UNKNOWN": "raises"},
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = commands.add_parser("run", help="score both models and write the V3 all-AND gate")
    ...
    validate = commands.add_parser("validate-gate", help="validate a V3 aggregate PASS artifact")
