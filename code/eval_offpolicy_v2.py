#!/usr/bin/env python3
"""Evaluate frozen T2 with an all-AND-gated off-policy V2 checkpoint.
...
the reproducible aggregate PASS gate from ``evaluate_cube_offpolicy_v2.py``;
two-environment smoke outputs are isolated below ``offpolicy_v2/smoke``.
"""
...
import evaluate_cube_offpolicy_v2 as offline  # noqa: E402
...
def _checkpoint_provenance(checkpoint: Path) -> dict[str, Any]:
    ...
    if not config.is_file():
        raise FileNotFoundError(f"V2 checkpoint config missing: {config}")
    return {
        "kind": "offpolicy_v2_planner_distribution_multistep_finetune",
...
def run(args: argparse.Namespace) -> int:
    ...
    payload = {
        "format_version": "cube_offpolicy_v2_t2_evaluation_v1",
        "protocol": {
            "id": PROTOCOL,
